...
void Map::set(Coordinate *cm, Room *room) {
	...
	c->add(cm);
	vector<vector<vector<Room *> > > & segment = getSegment(c);
	if (segment.capacity() < (unsigned int)c->x) segment.resize((unsigned int)c->x);
	if (segment[(unsigned int)c->x].capacity() < (unsigned int)c->y) segment[(unsigned int)c->x].resize((unsigned int)c->y);
	if (segment[(unsigned int)c->x][(unsigned int)c->y].capacity() < (unsigned int)c->z) segment[(unsigned int)c->x][(unsigned int)c->y].resize((unsigned int)c->z);
