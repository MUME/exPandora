...
SearchTreeNode::SearchTreeNode(ParseEvent * event, TinyList<RoomSearchNode *> * in_children) {
  ...
  if (in_children == 0) children = new TinyList<RoomSearchNode *>();
...
AbstractRoomContainer * SearchTreeNode::getRooms(ParseEvent * event) {
  ...
  selectedChild = children->get((unsigned char)event->current()->next());
